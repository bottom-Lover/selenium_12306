...
class hackTickets(object):
    ...
    def login(self):
        ...
        # 自动填充用户名
        elemName = self.driver.find_element_by_name("loginUserDTO.user_name")
        elemName.send_keys(self.username)
        # 自动填充密码
        elemPsd = self.driver.find_element_by_name("userDTO.password")
        elemPsd.send_keys(self.passwd)
    ...
    def searchMore(self):
        ...
        for type in self.train_types:
            ...
            if type == 'T' or type == 'G' or type == 'D' or type == 'Z' or type == 'K':
                ...
                # WebDriverWait(self.driver,10).until(EC.text_to_be_present_in_element((By.LINK_TEXT,train_type_dict[type])))
                self.driver.find_element_by_xpath('//*[text()="%s"]' % train_type_dict[type]).click()
            else:
        ...
        if self.start_time:
            self.driver.find_element_by_link_text(self.start_time).click()
        else:
    ...
    def specifyTrainNo(self):
        ...
        while self.driver.current_url == self.ticket_url:
            # sleep(0.05 + random.randint(3,4))
            WebDriverWait(self.driver,10).until(EC.element_to_be_clickable((By.LINK_TEXT,"查询")))
            sleep(random.randint(1,3))
            self.driver.find_element_by_link_text(u"查询").click()
            count += 1
            print(u"循环点击查询... 第 %s 次" % count)
            try:
                WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.ID, "ZE_5e000G186621")))
                if self.driver.find_element_by_xpath('//*[@id="%s"][1]' % "ZE_5e000G186621").text != "候补" \
                        and  self.driver.find_element_by_xpath('//*[@id="%s"][1]' % "ZE_5e000G186621").text != "无" :
                    self.driver.find_elements_by_xpath('//*[text()="预订"]')[self.order - 1].click()
                    sleep(0.3)
    ...
    def buyOrderZero(self):
        ...
        while self.driver.current_url == self.ticket_url:
            ...
            try:
                for i in self.driver.find_element_by_link_text(u"预订"):
                    i.click()
                    # 等待0.3秒，提交等待的时间
                    sleep(0.3)
    ...
    def selUser(self):
        ...
        for user in self.users:
            self.driver.find_elements_by_xpath('//*[text()="%s"]' % user)[-1].click()
        # sleep(2)
    ...
    def confirmOrder(self):
        ...
        if self.seatType:
            sleep(1)
            try:
                l = self.driver.find_elements_by_xpath('//*[@value="%s"]' % self.seatType)
                print(l)
                if len(l) != 0:
                    l[0].click()
                else:
    ...
    def confirmSeat(self):
        ...
        l = self.driver.find_elements_by_link_text(self.seat_type + "余票<strong>0</strong>张")
        if list(l) == [] :
            print("****")
            ...
            if self.noseat_allow == 0:
                print("+++++++")
                self.driver.find_element_by_id('back_edit_id').click()
